...
// Weighted decoding graph metric.
// For your simulator, data errors exist but measurement noise is absent,
// so time-like/vertical connections should be more expensive.
constexpr int WX = 1;
constexpr int WT = 4;
...
void write_data_corrections_csv(
    ...
    for (const CorrectionChain &chain : result.chains) {
        for (int step = 1; step < static_cast<int>(chain.path.size()); ++step) {
            ...
            // Only horizontal path segments are data corrections.
            // Vertical segments are time-like and should not flip final data.
            if (t0 != t1) continue;
            if (x0 == x1) continue;
...
int main(int argc, char **argv) {
    ...
    summary << "p,shot,num_defects,success,final_radius,num_clusters,num_chains\n";
